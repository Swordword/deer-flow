...
For code-heavy Agent workflows, `repo_snapshot` prepares administrator-approved Git repositories as persistent, per-user/per-thread local snapshots. Configure repository aliases and `REPO_SNAPSHOT_TOKEN`, enable the `repo` tool group, then call `repo_prepare` once before analysis. Gateway maintains a locked bare-mirror cache under `.deer-flow/git-cache`, resolves the requested ref to a fixed commit SHA, and creates a self-contained clone under `/mnt/user-data/workspace/repos/<alias>`. M2/M3 can then use local `grep`, file reads, tests, and `git diff` instead of repeatedly fetching source through GitLab MCP. The standard `${DEER_FLOW_HOME}` Docker mount persists both cache and snapshots across container restarts; credentials remain in the Gateway Git subprocess environment and are not written to the snapshot or injected into the Sandbox.
