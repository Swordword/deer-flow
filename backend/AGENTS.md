...
- `agent_server.py` exposes the configured M1/M2/M3 product-engineering subagents as an external MCP server via the `deerflow-agent-mcp` console script. In addition to the three low-level stage tools, it exposes `start_product_engineering_pipeline`, `advance_product_engineering_pipeline`, and `get_product_engineering_pipeline_status`. `product_engineering_workflow.py` persists owner-partitioned JSON state under `runtime_home()/product-engineering-pipelines`, atomically serializes in-process advances, executes exactly one stage per advance, retains attempt and caller-declared approval audit records, and validates fenced YAML `AGENT_RESULT` handoffs before changing gates. HTTP clients can run it with `uv run deerflow-agent-mcp --transport streamable-http --host 0.0.0.0 --port 8003`. M2 requires PRD approval, M3 requires PRD plus tech-design approval, and non-dry-run M3 calls require `allowed_paths`. The standalone server does not authenticate caller-supplied user/approval values; multi-user exposure requires a trusted authentication proxy.
